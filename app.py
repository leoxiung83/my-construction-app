...
import re # [修正] 補上 re
from PIL import Image # [修正] 補上 Image
...
if sheet_status is None:
        ...
        with st.expander("👷 04. 人力與機具出工紀錄", expanded=True):
            ...
            with cols_g4[0]:
                ...
                if cat:
                    ...
                    # 1. 取得該類別的選單項目
                    options = current_items.get(cat, [])
                    cost_item = st.selectbox("項目", options, key=f"sel_{cat}_{d_key}")
                    ...
                    # 預設值 logic: 若沒設定則為 0 與 "工"
                    defaults = price_data[global_project][cat].get(cost_item, {'price': 0, 'unit': '工'})
                    
                    c_q, c_p = st.columns(2)
                    with c_q: 
                        cost_qty = st.number_input("數量", min_value=0.0, step=0.5, value=1.0, key=f"qty_{cat}_{d_key}_{cost_item}")
                    with c_p: 
                        # 這裡使用 defaults['price'] 作為 value，且 key 包含 cost_item，確保切換項目時會更新
                        cost_price = st.number_input("單價 ($)", value=float(defaults['price']), step=100.0, key=f"price_{cat}_{d_key}_{cost_item}")
                    
                    cost_unit = st.text_input("單位", value=defaults['unit'], key=f"unit_{cat}_{d_key}_{cost_item}")
                    cost_note = st.text_input("備註", key=f"note_{cat}_{d_key}_{cost_item}")
            ...
            with cols_g4[1]:
                ...
                if cat:
                    ...
                    defaults = price_data[global_project][cat].get(cost_item, {'price': 0, 'unit': '式'})
    ...
    with tab_data:
        ...
        if proj_df.empty:
            st.info(f"專案【{global_project}】尚無資料")
        else:
            ...
            def render_section(display_title, cats, key, cost=False, qty=False):
                ...
                if not sec_df.empty:
                    ...
                    if not view.empty:
                        ...
                        if cost:
                            col_cfg.update({"單價": st.column_config.NumberColumn(width="small"), "總價": st.column_config.NumberColumn(disabled=True, width="small")})
                        else:
                            col_cfg.update({"單價": None, "總價": None})
                            
                        if qty:
                            col_cfg.update({"數量": st.column_config.NumberColumn(width="small"), "單位": st.column_config.TextColumn(width="small")})
                        else:
                            col_cfg.update({"數量": None, "單位": None})
                                ...
                                with col_show:
                                    if img_filename:
                                        ...
                                        if os.path.exists(img_path):
                                            ...
                                            except: st.error("照片檔案毀損")
                                        else: st.warning(f"⚠️ 找不到照片檔案：{img_filename}")
                                    else: st.info("ℹ️ 此項目目前【沒有照片】")

                                with col_manage:
                                    st.write(f"🔧 **管理操作 ({row_name})**")
                                    if img_filename and st.button("🗑️ 刪除此照片", key=f"del_img_{index}"):
                        ...
                        if st.session_state[sk]: 
                            ...
                            with cy:
                                if st.button("✔️ 是", key=f"y_{key}", type="primary"): 
                                    ...
                                    st.rerun()
                            with cn:
                                if st.button("❌ 否 (取消)", key=f"n_{key}"):
                                    st.session_state[sk] = False
                                    st.rerun()
            ...
            for i, (cat, display_name) in enumerate(sorted_cats):
                ...
                with col:
                    ...
                    with st.expander("編輯"):
                        ...
                        # [新增功能] 針對 工種(人力) 與 機具(設備) 增加單價設定
                        if cat in COST_CATEGORIES:
                            st.markdown("##### 💰 預設單價設定")
                            for item in curr_list:
                                c_p, c_u, c_b = st.columns([2, 1, 1])
                                # 取得現有設定
                                current_setting = price_data[global_project][cat].get(item, {'price': 0, 'unit': '工' if '工種' in cat else '式'})
                                with c_p:
                                    new_price = st.number_input(f"{item} 單價", value=float(current_setting['price']), step=100.0, key=f"price_set_{cat}_{item}")
                                with c_u:
                                    new_unit = st.text_input(f"單位", value=current_setting['unit'], key=f"unit_set_{cat}_{item}")
                                with c_b:
                                    st.write("") # Spacer
                                    if st.button("✅", key=f"save_price_{cat}_{item}"):
                                        price_data[global_project][cat][item] = {'price': new_price, 'unit': new_unit}
                                        save_prices(price_data)
                                        st.toast(f"{item} 設定已儲存")
                            st.divider()

                        target = st.selectbox("選擇項目", curr_list, key=f"tgt_{cat}")
                        ...
                        with c_d:
                            if st.button("移除", key=f"btn_d_{cat}"):

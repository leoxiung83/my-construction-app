...
# 0. 系統設定
# ==========================================
st.set_page_config(page_title="多專案施工管理系統 (線上完美版)", layout="wide", page_icon="☁️")

# --- 讀取 Secrets (密碼與雲端設定) ---
# 確保在 Streamlit Cloud 後台設定這些 Secrets
        ...
        def render_online_section(cat_key, cat_disp, cat_type, key):
            ...
            if not sec_df.empty:
                ...
                if not view.empty:
                    ...
                    # 隱藏原始備註，只顯示處理過的
                    # [修復重點] 將原本的 column_config(hidden=True) 改為直接 drop，再用索引對應
            ...
            with c5:
                if st.button("💾", key=f"save_{target_key}_{item}"):
                    if new_name_input != item:
                        update_item_name(global_project, target_key, item, new_name_input, settings_data)
                    if cat_type == 'cost':

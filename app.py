...
HOLIDAYS = {
    ...
    2025-02-28 和平紀念日, 2025-04-04 兒童節清明節, 2025-05-01 勞動節, 2025-05-31 端午節,
...
def save_dataframe(df)
    ...
    df_save = df_save.drop(columns=[c for c in ['月份', '刪除', 'temp_month', '星期節日', '🗓️ 星期節日'] if c in df_save.columns])
    df_save['日期'] = df_save['日期'].astype(str)
    try
        sheet.clear(); sheet.update([df_save.columns.values.tolist()] + df_save.values.tolist())
    except Exception as e st.error(f雲端存檔失敗 {e})
...
with st.sidebar
    ...
    if global_date != st.session_state.last_check_date
        ...
        components.html(scriptvar tabs=window.parent.document.querySelectorAll('[data-testid=stTab]');if(tabs.length0){tabs[0].click();}script, height=0, width=0)
...
# === Tab 1 快速日報輸入 (維持原狀) ===
with tab_entry
    st.info(f正在填寫：{global_project}  {global_date})
        ...
        with st.expander(f📝 {CAT_CONFIG_LIST[0]['display']} 及 {CAT_CONFIG_LIST[1]['display']}, expanded=True)
            ...
            for i in range(2)
                ...
                with cols[i]
                    ...
                    it = st.selectbox(項目, opts if opts else [(請先至設定頁新增項目)], key=fs_{i}_{d_key})
                    p_set = price_data.get(global_project, {}).get(conf[key], {}).get(it, {price 0, unit 式})
                    with st.form(key=ff_{i}_{d_key})
                        tx = st.text_area(內容, height=100, key=fa_{i}_{d_key})
    ...
    if len(CAT_CONFIG_LIST) = 3
        ...
        with st.expander(f🚛 {conf['display']}, expanded=True)
            ...
            for k in range(3)
                with cols[k]
                    it = st.selectbox(材料, opts if opts else [(請先新增項目)], key=fis_{k}_{d_key})
                    p_set = price_data.get(global_project, {}).get(conf[key], {}).get(it, {price 0, unit 式})
                    with st.form(key=ff_2_{k}_{d_key})
                        q = st.number_input(數量, min_value=0.0, step=1.0, key=fiq_{k}_{d_key})
                        u = st.text_input(單位, value=p_set[unit], key=fiu_{k}_{d_key}_{it})
    ...
    if len(CAT_CONFIG_LIST) = 4
        ...
        with st.expander(f🧱 {conf['display']}, expanded=True)
            ...
            for k in range(3)
                with cols[k]
                    it = st.selectbox(材料, opts if opts else [(請先新增項目)], key=fus_{k}_{d_key})
                    p_set = price_data.get(global_project, {}).get(conf[key], {}).get(it, {price 0, unit m3})
                    with st.form(key=ff_3_{k}_{d_key})
                        q = st.number_input(數量, min_value=0.0, step=0.5, key=fuq_{k}_{d_key})
                        u = st.text_input(單位, value=p_set[unit], key=fuu_{k}_{d_key}_{it})
                        if st.form_submit_button(f💾 儲存 {k+1}) and opts
                            append_data(global_date, global_project, conf[key], conf[type], it, u, q, 0, ); st.rerun()
        ...
        with st.expander(👷 人力與機具出工紀錄, expanded=True)
            ...
            for i in [4, 5]
                ...
                with cols[i-4]
                    st.markdown(f### {conf['display']})
                    opts = current_items.get(conf[key], [])
                    it = st.selectbox(項目, opts if opts else [(請先新增項目)], key=fcs_{i}_{d_key})
                    p_set = price_data.get(global_project, {}).get(conf[key], {}).get(it, {price 0, unit 工 if i==4 else 式})
                    with st.form(key=ff_{i}_{d_key})
                        cq, cp = st.columns(2)
                        q = cq.number_input(數量, value=1.0, step=0.5, key=fcq_{i}_{d_key})
                        p = cp.number_input(單價, value=float(p_set[price]), key=fcp_{i}_{d_key}_{it})
                        u = st.text_input(單位, value=p_set[unit], key=fcu_{i}_{d_key}_{it})
    ...
    for conf in CAT_CONFIG_LIST
        if conf[key] not in handled_keys
            with st.expander(f📌 {conf['display']}, expanded=True)
                opts = current_items.get(conf[key], [])
                if opts
                    it = st.selectbox(選擇項目, opts, key=fds_{conf['key']})
                    p_set = price_data.get(global_project, {}).get(conf[key], {}).get(it, {price 0, unit 式})
                    with st.form(key=fdyn_{conf['key']}_{d_key})
                        if conf[type] == 'text' tx = st.text_area(內容內容, key=fdt_{conf['key']}); q, p, u = 1, 0, p_set[unit]
                        else
                            c1, c2, c3 = st.columns(3)
                            q = c1.number_input(數量, value=1.0, key=fdq_{conf['key']})
                            p = c2.number_input(單價, value=float(p_set[price]), key=fdp_{conf['key']}_{it}) if conf[type] == 'cost' else 0
                            u = c3.text_input(單位, value=p_set[unit], key=fdu_{conf['key']}_{it}); tx = 
                        if st.form_submit_button(💾 儲存資料)
                            append_data(global_date, global_project, conf[key], conf[type], it, u, q, p, tx); st.rerun()

# === Tab 2 報表總覽 (修正：進料用料區塊新增「數量」並排在「單位」之後) ===
with tab_data
    proj_df = df[df['專案'] == global_project].copy()
    if proj_df.empty st.info(f專案【{global_project}】無資料)
    else
        ...
        def render_section(cat_key, cat_disp, cat_type, key)
            ...
            if not sec_df.empty
                ...
                if not view.empty
                    view['🗓️ 星期節日'] = view['日期'].apply(lambda x get_date_info(x)[0])
                    if '刪除' not in view.columns view.insert(0, 刪除, False)
                    
                    # 🌟 欄位過濾與排序邏輯 🌟
                    if cat_disp.startswith(01.) or cat_disp.startswith(02.)
                        # 文字類：不顯示單位、單價、數量、總額
                        cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '備註']
                    elif cat_disp.startswith(03.) or cat_disp.startswith(04.)
                        # 數量類 (進料用料)：顯示名稱、單位、數量 (數量排在單位後)、備註
                        cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '單位', '數量', '備註']
                    elif cat_disp.startswith(05.) or cat_disp.startswith(06.)
                        # 成本類：全顯示
                        cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '數量', '單位', '單價', '總價', '備註']
                    else
                        # 自定義項依照數據類型過濾
                        if cat_type == 'text' cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '備註']
                        elif cat_type == 'usage' cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '單位', '數量', '備註']
                        else cols_to_show = ['刪除', '日期', '🗓️ 星期節日', '名稱', '數量', '單位', '單價', '總價', '備註']
                    ...
                    col_cfg = {
                        ...
                        🗓️ 星期節日 st.column_config.TextColumn(disabled=True, width=medium),
                    ...
                    edited = st.data_editor(view_final.sort_values('日期', ascending=False), key=fe_{key}, column_config=col_cfg, use_container_width=True, hide_index=True)
                    
                    b1, b2, _ = st.columns([1, 1, 6])
                    with b1 
                        if st.button(💾 更新修改, key=fs_{key}) 
                            target_indices = edited.index
                            common_cols = [c for c in edited.columns if c in df.columns and c not in ['刪除', '🗓️ 星期節日']]
                    ...
                    with b2 
                        if st.button(🗑️ 刪除選取, key=fd_{key}, type=primary) 
                            if not edited[edited['刪除']].empty st.session_state[sk] = True
                    if st.session_state[sk] 
                        st.warning(確定刪除？)
                        cy, cn = st.columns(2)
                        with cy
                            if st.button(✔️ 是, key=fy_{key})
                                delete_indices = edited[edited['刪除']].index
                                df.drop(delete_indices, inplace=True)
                                save_dataframe(df); st.session_state[sk] = False; st.rerun()
                        with cn
                            if st.button(❌ 否, key=fn_{key}) st.session_state[sk] = False; st.rerun()

        for config in CAT_CONFIG_LIST
            render_section(config[key], config[display], config[type], fsec_{config['key']})
...
with tab_settings
    ...
    with st.expander(📦 資料備份中心, expanded=False)
        st.download_button(📦 下載完整備份 (ZIP), create_zip_backup(), file_name=fbackup_{datetime.now().strftime('%Y%m%d')}.zip, mime=applicationzip)
        uploaded_file = st.file_uploader(📤 系統還原 (ZIPCSVJSON), type=['csv', 'zip', 'json'])
        if uploaded_file and st.button(⚠️ 確認執行還原)
            try
                if uploaded_file.name.endswith('.json')
                    ...
                    st.success(f設定檔還原成功！); time.sleep(1); st.rerun()
            ...
            except Exception as e st.error(f還原失敗：{e})
    ...
    with st.expander(2. 新增管理項目 (新增大標題), expanded=False)
        ...
        n_bt = c2.selectbox(類型, [text, usage, cost], format_func=lambda x {text文字,usage數量,cost成本}[x])
    ...
    with st.expander(3. 既有選單項目管理 (修改大標題  細項內容), expanded=True)
        ...
        for i, conf in enumerate(CAT_CONFIG_LIST)
            ...
            c1.text(f原 {conf['display']})
            nd_in = c2.text_input(f新標題 {i}, value=conf['display'], label_visibility=collapsed)
            if nd_in != conf['display'] and st.button(更新, key=fu_{i}) update_category_config(i, nd_in, settings_data); st.rerun()
            if c4.button(🗑️, key=fd_{i}) delete_category_block(i, settings_data); st.rerun()
        ...
        if t_conf
            ...
            ni_in = c_a.text_input(f在【{target_v}】新增項目內容, key=fno_{tk})
            if c_b.button(➕ 加入項目, key=fba_{tk}) and ni_in current_items[tk].append(ni_in); save_settings(settings_data); st.rerun()
            st.markdown(f目前項目清單 ({len(c_list)}))
            ...
            for it_v in c_list
                ...
                if ct == 'text'
                    ...
                    with r2 rnn_in = r2.text_input(RN, value=it_v, key=fr_{tk}_{it_v}, label_visibility=collapsed)
                    if r3.button(💾, key=fs_{tk}_{it_v})
                        if rnn_in != it_v update_item_name(global_project, tk, it_v, rnn_in, settings_data, price_data)
                        st.toast(已更新); st.rerun()
                    if r4.button(🗑️, key=fdl_{tk}_{it_v}) current_items[tk].remove(it_v); save_settings(settings_data); st.rerun()
                elif ct == 'usage'
                    r1, r2, r3, r4, r5 = st.columns([2, 2, 2, 1, 1])
                    with r1 st.text(it_v)
                    with r2 rnn_in = r2.text_input(RN, value=it_v, key=fr_{tk}_{it_v}, label_visibility=collapsed)
                    with r3 nu_in = r3.text_input(U, value=p_i[unit], key=fu_{tk}_{it_v}, label_visibility=collapsed)
                    if r4.button(💾, key=fs_{tk}_{it_v})
                        if rnn_in != it_v update_item_name(global_project, tk, it_v, rnn_in, settings_data, price_data)
                    ...
                    if r5.button(🗑️, key=fdl_{tk}_{it_v}) current_items[tk].remove(it_v); save_settings(settings_data); st.rerun()
                else
                    r1, r2, r3, r4, r5, r6 = st.columns([2, 2, 1, 1, 0.5, 0.5])
                    with r1 st.text(it_v)
                    with r2 rnn_in = r2.text_input(RN, value=it_v, key=fr_{tk}_{it_v}, label_visibility=collapsed)
                    with r3 np_in = r3.number_input(P, value=float(p_i[price]), key=fp_{tk}_{it_v}, label_visibility=collapsed)
                    with r4 nu_in = r4.text_input(U, value=p_i[unit], key=fu_{tk}_{it_v}, label_visibility=collapsed)
                    if r5.button(💾, key=fs_{tk}_{it_v})
                        if rnn_in != it_v update_item_name(global_project, tk, it_v, rnn_in, settings_data, price_data)
                        if tk not in price_data[global_project] price_data[global_project][tk] = {}
                        price_data[global_project][tk][rnn_in if rnn_in != it_v else it_v] = {price np_in, unit nu_in}; save_prices(price_data); st.rerun()
                    if r6.button(🗑️, key=fdl_{tk}_{it_v}) current_items[tk].remove(it_v); save_settings(settings_data); st.rerun()
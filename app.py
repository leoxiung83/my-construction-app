...
import re # [修正] 補上正則表達式套件
from PIL import Image # [修正] 補上圖片處理套件
...
if sheet_status is None:
        ...
        with st.expander("👷 04. 人力與機具出工紀錄", expanded=True):
            ...
            with cols_g4[0]:
                ...
                if cat:
                    ...
                    with c_p: 
                        # 使用 float() 確保數值正確，key 加上 cost_item 確保切換時更新
                        cost_price = st.number_input("單價 ($)", value=float(item_setting["price"]), step=100.0, key=f"price_{cat}_{d_key}_{cost_item}")
    ...
    with tab_dash:
        if df.empty:
            ...
            if dash_df.empty:
                st.warning(f"專案【{global_project}】目前沒有資料。")
            else:
                ...
                if not cost_df.empty:
                    ...
                    if not chart_data.empty:
                        ...
                        pie = base.mark_arc(outerRadius=100, innerRadius=50).encode(
                            color=alt.Color("類別"),
                            order=alt.Order("總價", sort="descending"),
                            tooltip=["類別", "總價"]
                        )
                        text = base.mark_text(radius=120).encode(
                            text=alt.Text("總價", format=",.0f"),
                            order=alt.Order("總價", sort="descending"),
                    ...
                    with st.expander("編輯"):
                        ...
                        if cat in COST_CATEGORIES:
                            ...
                            for item_name in curr_list:
                                ...
                                with c_p:
                                    # 修正：確保 value 是 float 類型
                                    new_p = st.number_input(f"{item_name} 單價", value=float(item_data["price"]), step=100.0, key=f"p_{cat}_{item_name}", label_visibility="collapsed")
                                with c_u:
                                    new_u = st.text_input(f"單位", value=item_data["unit"], key=f"u_{cat}_{item_name}", label_visibility="collapsed")
                                with c_b: 
                                    st.write("")
                                    st.write("")

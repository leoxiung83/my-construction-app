...
if not st.session_state.logged_in:
    st.markdown("## 🔒 系統鎖定"); st.markdown("請輸入密碼以繼續。")
    st.text_input("密碼：", type="password", key="password_input", on_change=check_login); st.stop()
...
def get_google_sheet():
    ...
    except gspread.SpreadsheetNotFound: return "NOT_FOUND"
    except Exception as e: st.error(f"Sheet 連線錯誤: {e}"); return None
...
def load_json(filepath, default_data):
    if not os.path.exists(filepath):
        ...
        return default_data
    try: with open(filepath, 'r', encoding='utf-8') as f: return json.load(f)
...
def load_settings():
    ...
    save_json(SETTINGS_FILE, data); return data

def save_settings(data): save_json(SETTINGS_FILE, data)
def load_prices(): data = load_json(PRICES_FILE, {}); save_json(PRICES_FILE, data); return data
...
def load_data():
    ...
    if sheet == "NOT_FOUND": st.error(f"找不到雲端試算表：{SHEET_NAME}。"); return pd.DataFrame(columns=cols)
    elif sheet is None: st.warning("⚠️ 未偵測到金鑰。"); return pd.DataFrame(columns=cols)
    try:
        ...
        df['類別'] = df['類別'].replace({'本日施工概況': '施工說明', '01.本日施工概況': '施工說明', '現場文字紀錄': '相關紀錄', '相關紀錄(會議、會勘、走動管理等)': '相關紀錄'})
        df['日期'] = pd.to_datetime(df['日期'], errors='coerce'); df = df.dropna(subset=['日期']); df['日期'] = df['日期'].dt.date
        df['月份'] = pd.to_datetime(df['日期']).dt.strftime("%Y-%m")
        for col in ['總價', '數量', '單價']: df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
        return df
    except Exception as e: st.error(f"讀取錯誤: {e}"); return pd.DataFrame(columns=cols)
...
def save_dataframe(df):
    try:
        ...
        df_save = df.drop(columns=cols_drop); df_save['日期'] = df_save['日期'].astype(str)
        sheet.clear(); sheet.update([df_save.columns.values.tolist()] + df_save.values.tolist())
    except Exception as e: st.error(f"存檔錯誤: {e}")
...
def append_data(date, project, category, name, unit, qty, price, note, image_file=None):
    ...
    if image_file is not None:
        ...
        if img_url:
            # 將連結附在備註後方
            if note: note += f" (圖: {img_url})"
            else: note = f"(圖: {img_url})"
        else:
    ...
    except Exception as e: st.error(f"寫入錯誤: {e}")
...
def rename_project_logic(old_name, new_name, settings, prices):
    ...
    idx = settings["projects"].index(old_name); settings["projects"][idx] = new_name
    ...
    df = load_data()
    if not df.empty: df.loc[df['專案'] == old_name, '專案'] = new_name; save_dataframe(df)
    return True, "成功"
...
def rename_item_in_project(project, category, old_item, new_item, settings, prices):
    ...
    if project in prices and category in prices[project] and old_item in prices[project][category]:
        ...
        save_prices(prices)
    df = load_data()
    if not df.empty: df.loc[(df['專案']==project) & (df['類別']==category) & (df['名稱']==old_item), '名稱'] = new_item; save_dataframe(df)
    save_settings(settings); return True

def create_zip_backup():
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        df = load_data(); csv_buffer = io.StringIO()
...
if sheet_status is None:
    st.warning("⚠️ 系統未偵測到金鑰。請確認 service_key.json (電腦) 或 Secrets (手機) 已設定。")
else:
    with st.sidebar:
        st.header("📅 日期與專案")
        ...
        if global_project not in settings_data["items"]: settings_data["items"][global_project] = copy.deepcopy(DEFAULT_TEMPLATE); save_settings(settings_data)
        current_items = settings_data["items"][global_project]
        st.divider()
        if st.button("🔄 強制重新整理資料"): st.cache_resource.clear(); st.rerun()
                ...
                if real_cat:
                    ...
                    with st.form(key=f"form_status_{d_key}"):
                        ...
                        if st.form_submit_button("💾 儲存說明"):
                            ...
                            st.toast("已儲存，同步中..."); time.sleep(1.5); st.rerun()
            with cols_g1[1]:
    ...
    with tab_data:
        ...
        if proj_df.empty: st.info(f"專案【{global_project}】尚無資料")
            ...
            def render_section(display_title, cats, key, cost=False, qty=False):
                ...
                if not sec_df.empty:
                    ...
                    if not view.empty:
                        ...
                        cols = list(view.columns); cols.insert(1, cols.pop(cols.index('🗓️ 星期/節日'))); view = view[cols]
                        ...
                        with b1: 
                            if st.button("💾 更新修改", key=f"s_{key}"): 
                                vis = edited.drop(columns=['刪除']); merged = pd.concat([hidden, vis], ignore_index=True)
                                final = update_by_scope(df, merged, global_project, ed_month, cats); save_dataframe(final); st.toast("更新成功，同步中..."); time.sleep(1.5); st.rerun()
                        with b2: 
                        ...
                        if st.session_state[sk]: 
                            ...
                            with cy:
                                if st.button("✔️ 是", key=f"y_{key}", type="primary"): 
                                    vis = edited[~edited['刪除']].drop(columns=['刪除']); merged = pd.concat([hidden, vis], ignore_index=True)
                                    final = update_by_scope(df, merged, global_project, ed_month, cats); save_dataframe(final); st.session_state[sk] = False; st.toast("刪除成功，同步中..."); time.sleep(1.5); st.rerun()
                            with cn:
            ...
            for base_key, display_name in ORDER_MAP.items():
                ...
                if target_cats:
                    is_cost = "工種" in base_key or "機具" in base_key; is_qty = "進料" in base_key or "用料" in base_key or is_cost
    ...
    with tab_dash:
        if df.empty: st.info("無資料")
            ...
            if dash_df.empty: st.warning(f"專案【{global_project}】目前沒有資料。")
                ...
                d_cost = dash_df[dash_df['日期'] == today_str]['總價'].sum(); m_cost = dash_df[dash_df['月份'] == cur_month]['總價'].sum(); t_cost = dash_df['總價'].sum()
                k1, k2, k3 = st.columns(3)
                k1.metric("今日費用", f"${d_cost:,.0f}"); k2.metric("本月費用", f"${m_cost:,.0f}"); k3.metric("專案總費用", f"${t_cost:,.0f}"); st.divider()
                ...
                if not cost_df.empty:
                    ...
                    if not chart_data.empty:
                        ...
                        st.altair_chart(pie + text, use_container_width=True); st.divider()
                        col_man, col_mach = st.columns(2)
                        with col_man:
                            st.markdown("### 👷 人力費用明細"); man_data = chart_data[chart_data['類別'].str.contains("工種")]
                            if not man_data.empty:
                                man_bar = man_data.groupby('名稱')['總價'].sum().reset_index()
                                st.bar_chart(man_bar, x='名稱', y='總價', color="#FF6C6C"); st.dataframe(man_data[['日期', '名稱', '數量', '單價', '總價']], use_container_width=True, hide_index=True); st.markdown(f"**人力總計: ${man_data['總價'].sum():,.0f}**")
                            else: st.info("無人力資料")
                        with col_mach:
                            st.markdown("### 🚜 機具費用明細"); mach_data = chart_data[chart_data['類別'].str.contains("機具")]
                            if not mach_data.empty:
                                mach_bar = mach_data.groupby('名稱')['總價'].sum().reset_index()
                                st.bar_chart(mach_bar, x='名稱', y='總價', color="#4B8BBE"); st.dataframe(mach_data[['日期', '名稱', '數量', '單價', '總價']], use_container_width=True, hide_index=True); st.markdown(f"**機具總計: ${mach_data['總價'].sum():,.0f}**")
                            else: st.info("無機具資料")
    ...
    with tab_settings:
        ...
        with st.expander("📦 資料備份中心", expanded=False):
            ...
            if uploaded_file and st.button("⚠️ 確認還原"):
                try:
                    if uploaded_file.name.endswith('.csv'): df_restore = pd.read_csv(uploaded_file); save_dataframe(df_restore); st.success("CSV 資料還原成功！同步中..."); time.sleep(1.5); st.rerun()
                    elif uploaded_file.name.endswith('.zip'):
                        with zipfile.ZipFile(uploaded_file, 'r') as z: z.extractall("."); 
                        if DATA_FILE in z.namelist(): df_restore = pd.read_csv(DATA_FILE); save_dataframe(df_restore)
                        st.success("完整系統還原成功！同步中..."); time.sleep(1.5); st.rerun()
            ...
            with c2:
                ...
                if st.button("✏️ 確認改名"):
                    if ren_p != global_project:
                        ...
                        if suc: st.session_state.mem_project = ren_p; st.success(msg); time.sleep(1); st.rerun()
                        else: st.error(msg)
            with c3:
                st.markdown("**移除專案**")
                st.write("")
        ...
        with st.expander("2. 匯入/複製 專案設定 (範本)", expanded=False):
            ...
            if st.button("📥 確認匯入", disabled=not confirm_import):
                ...
                settings_data["items"][global_project] = copy.deepcopy(src_items); save_settings(settings_data); st.success("匯入成功"); time.sleep(1); st.rerun()
        st.subheader("3. 獨立選單與預設單價/單位"); st.caption(f"正在設定：**{global_project}**")
                    ...
                    with st.expander("編輯"):
                        ...
                        if cat in COST_CATEGORIES:
                            ...
                            for item_name in curr_list:
                                ...
                                with c_b: 
                                    st.write(""); st.write("")
                                    if st.button("✅", key=f"set_{cat}_{item_name}"): price_data[global_project][cat][item_name] = {"price": new_p, "unit": new_u}; save_prices(price_data); st.toast(f"已儲存：{item_name}")
                            st.write("---")

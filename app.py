...
# --- 讀取 Secrets (密碼與雲端設定) ---
# 注意：這些設定必須在 Streamlit Cloud 的 Secrets 裡面設定，不要寫死在程式碼
SYSTEM_PASSWORD = st.secrets.get("SYSTEM_PASSWORD", "12345") 
IMAGE_FOLDER_ID = st.secrets.get("IMAGE_FOLDER_ID", "")
SHEET_NAME = "construction_db"          # 您的 Google 試算表名稱
CONFIG_SHEET_NAME = "System_Config"     # 用來存設定的分頁名稱

# --- 台灣例假日 ---
HOLIDAYS = {
    "2025-01-01": "元旦", "2025-01-27": "小年夜", "2025-01-28": "除夕", "2025-01-29": "春節", 
    "2025-01-30": "初二", "2025-01-31": "初三", "2025-02-28": "和平紀念日", 
    "2025-04-04": "兒童節/清明節", "2025-05-01": "勞動節", "2025-05-31": "端午節",
...
def upload_image_to_drive(image_file, filename):
    ...
    if not IMAGE_FOLDER_ID:
        st.error("請在 Secrets 設定 IMAGE_FOLDER_ID")
        return None
    try:
...
# --- 設定檔讀寫 (存於 Google Sheet: System_Config) ---
def load_settings_online():
    client = get_google_client()
    if not client: return None
    try:
        sh = client.open(SHEET_NAME)
        # 嘗試讀取 System_Config 分頁
        try:
...
def save_settings_online(data):
    ...
    try:
        ...
        st.error(f"儲存設定檔失敗: {e}")

# --- 資料讀寫 (存於 Google Sheet: Sheet1) ---
...
def update_sheet_data_online(df):
    ...
    try:
        ...
        cols_drop = [c for c in ['月份', 'temp_month', '星期/節日'] if c in df_save.columns]
        df_save = df_save.drop(columns=cols_drop)
...
# --- 管理功能邏輯 ---
def update_item_name_manager(project, category, old_name, new_name, settings):
    if old_name == new_name: return
    
    # 1. 更新選單
    curr_list = settings["items"][project].get(category, [])
    if new_name in curr_list: return # 避免重複
...
def update_cat_display_manager(idx, new_name, settings):
    settings["cat_config"][idx]["display"] = new_name
    save_settings_online(settings)
    return True

def add_new_cat_manager(key, display, type_val, settings):
    # 檢查 key 是否重複
    for c in settings["cat_config"]:
        if c["key"] == key: return False
    
    settings["cat_config"].append({"key": key, "display": display, "type": type_val})
    
    # 為所有專案補上這個 key
    for proj in settings["items"]:
        if key not in settings["items"][proj]:
            settings["items"][proj][key] = []
    ...
    def process_append(cat_key, cat_type, name, unit, qty, price, note, img_file):
        ...
        if img_file:
            with st.spinner("📸 照片上傳中..."):
                ...
                fname = f"{timestamp}_{safe_name}.jpg"
        ...
        with st.expander(f"📝 {configs[0]['display']} 及 {configs[1]['display']}", expanded=True):
            ...
            with cols_g1[0]:
                ...
                with st.form(key=f"f_{idx}_{d_key}"):
                    ...
                    im = st.file_uploader("照", type=['jpg','png'], key=f"m_{idx}")
            ...
            for k in range(3):
                with cols_g2[k]:
                    ...
                    with st.form(key=f"f_{idx}_{k}_{d_key}"):
                        ...
                        im = st.file_uploader("照", type=['jpg','png'], key=f"m_{idx}_{k}")
            ...
            with c5:
                if st.button("💾", key=f"save_{target_key}_{item}"):
                    if new_name_input != item:
                        update_item_name_manager(global_project, target_key, item, new_name_input, settings_data)
                    if cat_type == 'cost':

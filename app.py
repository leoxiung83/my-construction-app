        ...
        def render_section(cat_key, cat_disp, cat_type, key):
            ...
            if not sec_df.empty:
                ...
                if not view.empty:
                    ...
                    # 恢復原本的 st.data_editor 設定，不限制顯示欄位
                    edited = st.data_editor(view.sort_values('日期', ascending=False), key=f"e_{key}", use_container_width=True, hide_index=True)

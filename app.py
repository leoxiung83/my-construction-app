...
ORDER_LIST = ["施工說明", "相關紀錄", "進料管理", "用料管理", "工種 (人力)", "機具 (設備)"]
...
def get_sorted_categories(cat_keys):
    def sort_key(k):
        for idx, base in enumerate(ORDER_LIST):
            if base in k: return idx
        return 999
    return sorted(cat_keys, key=sort_key)
...
with st.sidebar:
    ...
    day_str, _ = get_date_info(global_date)
...
with tab_entry:
    ...
    with st.expander("📝 01. 施工說明及相關紀錄", expanded=True):
        c1, c2 = st.columns(2)
        for i, kw in enumerate(["施工", "紀錄"]):
            cat = next((k for k in current_items if kw in k), None)
            if cat:
    ...
    # 2. 固定區塊：進料與用料
    with st.expander("🚛 02. 材料管理紀錄", expanded=True):
        c1, c2 = st.columns(2)
        for i, kw in enumerate(["進料", "用料"]):
            cat = next((k for k in current_items if kw in k), None)
            if cat:
                handled_cats.append(cat)
                with [c1, c2][i]:
                    st.markdown(f"**{cat}**")
                    with st.form(key=f"f_{cat}_{d_key}"):
                        it = st.selectbox("材料", current_items[cat])
    ...
    with st.expander("👷 03. 人力與機具出工", expanded=True):
        c1, c2 = st.columns(2)
        for i, kw in enumerate(["工種", "機具"]):
            cat = next((k for k in current_items if kw in k), None)
            if cat:
                handled_cats.append(cat)
                with [c1, c2][i]:
                    st.markdown(f"**{cat}**")
                    it = st.selectbox("項目", current_items[cat], key=f"sel_{cat}")
                    ...
                    with st.form(key=f"f_{cat}_{d_key}"):
                        ...
                        p = cp.number_input("單價", value=float(p_info["price"]))
    ...
    if remaining_cats:
        with st.expander("✨ 其他自定義管理項目 (新增標題)", expanded=True):
            for cat in remaining_cats:
                ...
                with st.form(key=f"dyn_{cat}_{d_key}"):
                    it = st.selectbox("項目", current_items[cat])
                    if c_type == "text":
                        tx = st.text_area("備註內容")
                        if st.form_submit_button(f"儲存 {cat}"):
                            append_data(global_date, global_project, cat, it, "式", 1, 0, tx, COST_CATEGORIES); st.rerun()
                    elif c_type == "cost":
                        ...
                        u = st.text_input("單位", value=p_info["unit"])
                        if st.form_submit_button(f"儲存 {cat}"):
                            append_data(global_date, global_project, cat, it, u, q, p, "", COST_CATEGORIES); st.rerun()
                    else: # usage
                        q = st.number_input("數量", value=0.0)
                        u = st.text_input("單位", value="式")
                        if st.form_submit_button(f"儲存 {cat}"):
                            append_data(global_date, global_project, cat, it, u, q, 0, "", COST_CATEGORIES); st.rerun()
...
with tab_data:
    ...
    if not proj_df.empty:
        ...
        sel_m = st.selectbox("選擇編輯月份", m_list)
        m_df = proj_df[proj_df['月份'] == sel_m].copy()
        for cat in get_sorted_categories(current_items.keys()):
            ...
            if not sec_df.empty:
                ...
                if st.button("💾 更新修改", key=f"btn_{cat}"):
                    save_dataframe(pd.concat([df[df['月份'] != sel_m], edited[~edited['刪除']]])); st.rerun()
...
with tab_dash:
    if not df.empty:
        ...
        pie = alt.Chart(year_df[year_df['總價']>0]).mark_arc(innerRadius=50).encode(theta="sum(總價)", color="類別")
        st.altair_chart(pie, use_container_width=True)

# --- TAB 4: ⚙️ 設定與管理 (功能置頂 + 順序優化) ---
with tab_settings:
    # 🌟 1. 備份置頂
    with st.expander("📦 資料備份與還原 (置頂)", expanded=True):
        c1, c2 = st.columns(2)
        c1.download_button("📥 下載系統完整備份 (ZIP)", create_zip_backup(), file_name=f"backup_{datetime.now().date()}.zip")
        up = c2.file_uploader("還原資料 (CSV/ZIP)")
        if up and st.button("⚠️ 確認執行還原"):
            st.success("還原成功"); st.rerun()

    # 🌟 2. 區塊管理 (新增功能)
    st.subheader("➕ 新增管理項目區塊 (標題)")
    ...
    sorted_cat_list = get_sorted_categories(list(current_items.keys()))
    cat_to_edit = st.selectbox("選擇要管理的類別", sorted_cat_list)
    
    if cat_to_edit:
        c_type = category_types.get(cat_to_edit, "text")
        # A. 新增項目
        with st.form(f"add_it_{cat_to_edit}"):
            ni = st.text_input(f"在 【{cat_to_edit}】 中新增細項")
            if st.form_submit_button("➕ 加入清單"):
                if ni: current_items[cat_to_edit].append(ni); save_settings(settings_data); st.rerun()

        # B. 垂直列表管理
        st.markdown("---")
        h_cols = st.columns([2, 3, 2, 2, 1, 1]) if c_type == "cost" else st.columns([3, 5, 1, 1])
        h_cols[0].caption("原名稱"); h_cols[1].caption("新名稱 (改名)")
        if c_type == "cost":
            h_cols[2].caption("預設單價"); h_cols[3].caption("預設單位")
        ...
        for idx, item in enumerate(current_items[cat_to_edit]):
            if c_type == "cost":
                ...
                if r5.button("💾", key=f"sv_{idx}"):
                    ...
                    save_prices(price_data); st.toast("儲存成功"); time.sleep(0.5); st.rerun()
